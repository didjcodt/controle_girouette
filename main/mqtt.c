...
static esp_err_t mqtt_event_handler(esp_mqtt_event_handle_t event) {
    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            msg_id =
                esp_mqtt_client_subscribe(client, CONFIG_MQTT_TOPIC_DISPLAY, 0);
            ESP_LOGI("MQTT_CLIENT",
                     "Subscribed to display topic (%s), msg_id=%d",
                     CONFIG_MQTT_TOPIC_DISPLAY, msg_id);

            msg_id =
                esp_mqtt_client_subscribe(client, CONFIG_MQTT_TOPIC_OTA, 0);
        ...
        case MQTT_EVENT_SUBSCRIBED:
            ESP_LOGD("MQTT_CLIENT", "MQTT_EVENT_SUBSCRIBED, msg_id=%d",
                     event->msg_id);
            break;

        case MQTT_EVENT_UNSUBSCRIBED:
            ESP_LOGD("MQTT_CLIENT", "MQTT_EVENT_UNSUBSCRIBED, msg_id=%d",
                     event->msg_id);
            break;

        case MQTT_EVENT_PUBLISHED:
            ESP_LOGD("MQTT_CLIENT", "MQTT_EVENT_PUBLISHED, msg_id=%d",
                     event->msg_id);
            break;
        ...
        case MQTT_EVENT_DATA:
            ...
            memset(&mqtt_string[event->data_len], 0,
                   MQTT_PAYLOAD_MAX_SIZE - event->data_len);
            mqtt_string[event->data_len] = '\0';

            if (memcmp(event->topic, CONFIG_MQTT_TOPIC_OTA, event->topic_len) ==
                0) {
                ESP_LOGI("MQTT_CLIENT", "OTA update!");
                xEventGroupSetBits(mqtt_event_group, MQTT_OTA_BIT);
            } else if (memcmp(event->topic, CONFIG_MQTT_TOPIC_DISPLAY,
                              event->topic_len) == 0) {
                ESP_LOGI("MQTT_CLIENT", "New string to display");
                new_string = 1;
            } else {
                ESP_LOGE("MQTT_CLIENT",
                         "Error, topic can be \"%s\" or \"%s\", but got \"%s\"",
