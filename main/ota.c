...
// Simple routine that waits for a publish in MQTT ota topic to upgrade firmware.
...
static void ota_upgrade_task(void* pvParameter) {
   ...
   while (xEventGroupWaitBits(
            mqtt_event_group, MQTT_OTA_BIT, false, true, portMAX_DELAY)) {
...
void ota_init() {
   ...
   xTaskCreateStatic(&ota_upgrade_task, "ota_upgrade_task", STACK_SIZE, NULL, tskIDLE_PRIORITY+1, ota_upgrade_task_stack, &ota_upgrade_task_buffer);
}
