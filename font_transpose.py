...
for char in font:
    ...
    for line in char:
        line_matrix = []
        bin_repr = '{0:08b}'.format(line)
        for bit in bin_repr:
            line_matrix.append(bit)
        char_matrix.append(line_matrix)
    matrix = np.array(char_matrix)
    matrix = matrix.T

    char_transposed = []
    for line in matrix:
        byte = ''.join(line)
        char_transposed.append(int(byte, 2))
    print(np.array(char_transposed));
